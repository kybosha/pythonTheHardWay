...
def enter_house():
	text_spreader(text.enter_house_text)
	downstairs_upstairs()
...
def up():
	text_spreader(text.upstairs_text)
	upstairs_room_choice = raw_input("> ")
	if "1" in upstairs_room_choice:
		text_spreader(text.room1_entry_text)
		room1()
	elif "2" in upstairs_room_choice:
		text_spreader(text.room2_entry_text)
		room2()
	elif "3" in upstairs_room_choice:
		text_spreader(text.room3_entry_text)
		room3()
...
def room1():
	text_spreader(text.room1_initial_text)
	room1_inspect = raw_input("> ")
	if "bed" in room1_inspect:
		text_spreader(text.room1_bed_text)
...
def room2():
	text_spreader(text.room2_initial_text)
...
# REMOVE AFTER TESTING
		...
		text_spreader(text.room2_no_fix_shower)
...
def room3():
	text_spreader(text.room3_initial_text)
	window_desk = raw_input("> ")
	if "window" in window_desk:
		text_spreader(text.room3_window_text)
		desk_leave = raw_input("> ")
		if "leave" in desk_leave:
			print "Heads back out to the landing."
			upstairs_hallway()
		elif "desk" in desk_leave:
			text_spreader(text.room3_window_desk_text)
			room3_lamp()
		else:
			print "That's not an option."
			room3()
	elif "desk" in window_desk:
		text_spreader(text.room3_desk_text)
		window_leave = raw_input("> ")
		if "window" in window_leave:
			text_spreader(text.room3_desk_window_text)
			upstairs_hallway()
		else:
			text_spreader(text.room3_whisper_text)
			upstairs_hallway()
...
def room3_lamp():
	...
	if lamp_on is True:
		text_spreader(text.room3_lamp_text)
		items.append(code)
		print items
...
def upstairs_hallway():
	...
	if "down" in upstairs_hallway_choice:
		text_spreader(text.upstairs_hallway_choice_down)
		downstairs()
...
def downstairs():
	text_spreader(text.downstairs_initial_text)
...
def car():
	...
	text_spreader(text.car_text2)
	print text.car_text2
